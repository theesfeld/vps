...
- Super+Shift+Return: if any PTYs exist, iced picker; Enter is `niri msg action spawn -- vps attach --id N`; that process execs the chosen terminal onto `ssh -tt grok vpsd attach` so the TTY owns its niri scope
